...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, heads = 8, dim_head = 64, dropout = 0., suppress = 0., thres = 1., alpha = 0.45):
        super().__init__()
    ...
    def forward(self, x, context = None, h = None):
        ...
        # Use context if provided, otherwise use x for both q and kv
        ctx = x if context is None else torch.cat((x, context), dim = 1)
        
        qkv = self.to_qkv(x).chunk(3, dim = -1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = heads), qkv)
        
        # For k and v, use context if provided
        if context is not None:
            kv_ctx = self.to_qkv(ctx).chunk(3, dim = -1)
            _, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = heads), kv_ctx)

        # raw attention
        raw = einsum('b h i d, b h j d -> b h i j', q, k) * self.scale
        
        # initialize h_prev for blending
        if h is None:
            h_prev = torch.randn_like(raw)
        else:
            h_prev = h

        # blend current and previous attention
...
class Transformer(nn.Module):
    def __init__(self, dim, num_patches, depth, heads, dim_head, mlp_dim_ratio, dropout = 0., stochastic_depth=0.,
        ...
        for i in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                dropout = dropout, suppress = suppress, thres = thres, alpha = 0.45)),
    ...
    def forward(self, x, context = None):
        ...
        for (attn, ff) in self.layers:
            ...
            attn_out, H = attn(x, context = context, h = H)
            x = self.drop_path(attn_out) + x
...
class PiT_mod_ver1(nn.Module):
    def __init__(self, *, img_size, patch_size, num_classes, dim, depth, heads, mlp_dim_ratio, dim_head = 64, dropout = 0., 
        ...
        super(PiT_mod_ver1, self).__init__()
