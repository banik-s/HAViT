...
class HybridAttention(nn.Module):
    ...
    def forward(self, x, layer_index):
        ...
        if layer_index < self.depth // 2:
            ...
            attn = hybrid_activation(dots, alpha=0.1, degree=2)  # Polynomial for deeper layers
    ...
    def flops(self, input_shape, layer_index):
        """Calculates FLOPS for the hybrid activation."""
        batch_size, seq_len, dim = input_shape
        num_elements = batch_size * self.heads * seq_len * seq_len #seq_len * seq_len is from the dots calculation.

        if layer_index < self.depth // 2:
            # ReLU FLOPS: Number of comparisons (approx. number of elements)
            return num_elements
        else:
            # Polynomial Activation FLOPS:
            # - ReLU: num_elements comparisons
            # - Power: num_elements multiplications
            # - Multiplication by alpha: num_elements multiplications
            # - Addition: num_elements additions
