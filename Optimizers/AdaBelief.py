...
class SAdaBelief(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        super(SAdaBelief, self).__init__(params, defaults)
        ...
        if amsgrad:
            print('AMS enabled in AdaBelief')
    def __setstate__(self, state):
        super(SAdaBelief, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['exp_avg_var'] = torch.zeros_like(p.data,
                                    memory_format=torch.preserve_format) if version_higher else torch.zeros_like(p.data)
                # Previous data
                state['previous_data'] = torch.zeros_like(p.data,
                                    memory_format=torch.preserve_format) if version_higher else torch.zeros_like(p.data)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # get current state variable
                exp_avg, exp_avg_var, previous_data = state['exp_avg'], state['exp_avg_var'], state['previous_data']
                ...
                # Update first and second moment running average
                diff_data = previous_data - p.data
                if state['step'] == 1:
                    exp_avg.mul_(beta1).add_(1 - beta1, grad)
                else:
                ...
                grad_residual = grad - exp_avg
                exp_avg_var.mul_(beta2).addcmul_(1 - beta2, grad_residual, grad_residual)
                state['previous_data'] = p.data.clone()
